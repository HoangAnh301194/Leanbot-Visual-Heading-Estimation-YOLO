...
class LostTrackingCollectorTests(unittest.TestCase):
    ...
    def test_label_check_draws_serialized_bbox_and_both_angles(self):
        ...
        with patch("lost_tracking_collector.cv2.rectangle", wraps=cv2.rectangle) as rectangle:
            with patch("lost_tracking_collector.cv2.putText", wraps=cv2.putText) as put_text:
                preview = render_label_check(image, "1 0.2 0.35 0.2 0.2\n", metadata)
        self.assertEqual(rectangle.call_args.args[1:3], ((64, 160), (192, 288)))
        captions = "\n".join(call.args[1] for call in put_text.call_args_list)
        self.assertIn("Class 1: Leanbot_p15", captions)
        self.assertIn("label angle: +15.0 deg", captions)
        self.assertIn("Source angle: +16.00 deg", captions)
        self.assertIn("source frame: 10", captions)
        self.assertIn("PSEUDO LABEL", captions)
        self.assertEqual(preview.shape, image.shape)
        np.testing.assert_array_equal(image, original_image)

    def test_label_check_arrow_uses_class_angle_with_image_y_axis(self):
        image = np.zeros((640, 640, 3), dtype=np.uint8)
        for class_angle, expected_end in ((0, (390, 320)), (90, (320, 250)),
                                          (180, (250, 320)), (-90, (320, 390))):
            with self.subTest(class_angle=class_angle):
                metadata = {
                    "class_name": "Leanbot_0", "class_angle_deg": class_angle,
                    "source_angle_deg": 45.0, "source_frame_id": 10,
                    "frame_id": 11, "frames_since_detection": 1,
                }
                with patch("lost_tracking_collector.cv2.arrowedLine", wraps=cv2.arrowedLine) as arrow:
                    render_label_check(image, "0 0.5 0.5 0.5 0.5\n", metadata)
                self.assertEqual(arrow.call_args.args[1:3], ((320, 320), expected_end))
